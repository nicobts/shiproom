...
Ground rules: `references/CHARTER.md` — binding, especially the verdict format, seat 2+
...
**2. Fact base (interactive).** Build `references/CHARTER.md`'s shared fact base:
...
`references/verdict.schema.json` (validate it; fix errors before finishing). Copy
`assets/dashboard.html` to `.council/index.html` so the verdict page is one `python3 -m
